...
const Badge: React.FC<{ status: FarmRow['status'] }> = ({ status }) => {
...
const AdminFarmManagement: React.FC = () => {
      ...
      <header className="bg-blue-600 text-white">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold">{t('farmMgmtTitle')}</h1>
              <p className="text-xs text-white/80">{t('farmMgmtSubtitle')}</p>
            </div>
          </div>
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <section className="bg-white rounded-xl shadow-sm p-4 border">
          <div className="flex items-center gap-2 text-gray-800 font-semibold mb-3">
            ...
            <span>{t('farmMgmtTitle')}</span>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
            <input value={query} onChange={(e)=>setQuery(e.target.value)} placeholder={t('farmName') + ' / ' + t('ownerNameLabel')} className="md:col-span-2 w-full px-4 py-3 rounded-lg border border-gray-300" />
            ...
            <select value={type} onChange={(e)=>setType(e.target.value)} className="w-full px-4 py-3 rounded-lg border border-gray-300">
              <option value="">{t('filterTypeLabel')}</option>
            ...
            <div className="md:col-span-5 flex gap-2 justify-between">
              <button onClick={reset} className="px-4 py-2 rounded-lg border bg-white hover:bg-gray-50 inline-flex items-center gap-2">
                <RefreshCw className="h-4 w-4" /> {t('resetFilters')}
              </button>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50 text-gray-600">
                <tr>
                  <th className="text-left px-4 py-3 font-medium">{t('colFarm')}</th>
                  ...
                  <tr key={f.id} className="border-t">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-2 justify-end">
                        ...
                        >
                          <Settings className="h-4 w-4" /> {t('manageAction')}
                        </button>
                      </div>
                    </td>
                  </tr>
