...
const MyFarms: React.FC = () => {
  ...
  const farms: Array<{ id: string; name: string; location: string }> = [
      ...
      <header className="bg-green-600 text-white">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold">{t('myFarms')}</h1>
          </div>
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        ...
        {farms.length === 0 ? (
          <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-dashed border-gray-300">
            <p className="text-gray-800 font-medium mb-1">{t('noFarmsYet')}</p>
            <p className="text-gray-500 mb-4">{t('registerNewFarm')}</p>
            <button
