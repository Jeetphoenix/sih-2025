...
const FarmDashboard: React.FC = () => {
  ...
  type TabType = 'logs' | 'visitors' | 'analytics' | 'medical' | 'qr';
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case 'logs':
        return (
          <div className="space-y-4">
            <h3 className="font-semibold text-gray-900">{t('dailyLogEntry')}</h3>
            <p className="text-sm text-gray-600">{t('recordTodaysFarm')}</p>
            <div className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-white p-4 rounded-lg border border-gray-200">
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t('numberOfDeaths')}</label>
                  <input type="number" className="w-full px-4 py-2 rounded-lg border border-gray-300" placeholder="0" />
                </div>
                <div className="bg-white p-4 rounded-lg border border-gray-200">
                  <label className="block text-sm font-medium text-gray-700 mb-2">{t('eggsCollected')}</label>
                  <input type="number" className="w-full px-4 py-2 rounded-lg border border-gray-300" placeholder="0" />
                </div>
              </div>
            </div>
      ...
      case 'visitors':
        return (
          <div className="space-y-4">
            <h3 className="font-semibold text-gray-900">{t('recentVisitors')}</h3>
            <div className="space-y-3">
              {[1, 2, 3].map((v) => (
                <div key={v} className="flex items-center justify-between bg-gray-50 border rounded-lg px-4 py-3">
                  <div>
                    <div className="font-medium">{t('visitorLabel')} {v}</div>
                    <div className="text-xs text-gray-500">
                      {t('individual')} · {t('checkedInAt')} 10:{30 + v} AM
                    </div>
                  </div>
        ...
        return (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-4">
              <h4 className="font-medium text-gray-900 mb-4">{t('dailyLogsCalendar')}</h4>
      ...
      case 'qr':
        return (
          <div className="space-y-6">
            <h3 className="font-semibold text-gray-900">{t('farmQrCode')}</h3>
            <p className="text-sm text-gray-600">{t('qrHelpText')}</p>
    ...
    <div className="min-h-screen bg-[#F6FAF6]">
      ...
      <header className="bg-[#F6FAF6] border-b border-gray-200">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-xl font-semibold text-gray-900">{t('farmDashboard')}</h1>
              <p className="text-xs text-gray-500">{t('welcomeBackUser')}</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <StatCard 
            label={t('totalLivestock')} 
            value={stats.livestock} 
            helper={`${stats.capacityPercent}% ${t('capacity')}`} 
            Icon={Activity} 
          />
          <StatCard 
            label={t('workersLabel')} 
            value={stats.workers} 
            helper={t('workersHelper')} 
            Icon={Users} 
          />
          <StatCard 
            label={t('todaysProduction')} 
            value={stats.productionToday} 
            helper={t('eggsCollected')} 
            Icon={Egg} 
          />
          <StatCard 
            label={t('todaysVisitors')} 
