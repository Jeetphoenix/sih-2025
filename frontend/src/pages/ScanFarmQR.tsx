...
// Minimal type for BarcodeDetector
// @ts-ignore
const QR_FORMATS = ['qr_code']

const ScanFarmQR: React.FC = () => {
      ...
      <header className="bg-purple-600 text-white">
        <div className="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-lg font-semibold">{t('scanFarmTitle')}</h1>
          </div>
      ...
      <main className="max-w-3xl mx-auto px-4 py-6 space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-sm p-4 border">
          <div className="flex items-center gap-2 text-gray-800 font-semibold">
            ...
            <span>{t('scanFarmTitle')}</span>
          </div>
          <p className="text-sm text-gray-500 mt-1">{t('scanSubtitle')}</p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            ...
            <button onClick={handleSwitchCamera} className="px-4 py-2 rounded-lg bg-white border hover:bg-gray-50 inline-flex items-center gap-2">
              <RefreshCw className="h-4 w-4" /> {t('switchCamera')}
            </button>
          </div>
        ...
        {result && (
          <div className="bg-white rounded-xl shadow-sm p-4 border space-y-3">
            <div className="text-sm text-gray-500">{t('decodedText')}</div>
            <div className="font-mono text-gray-900 break-all">{result}</div>
            <div className="flex flex-wrap gap-2">
              <button onClick={handleGoToFarm} className="px-4 py-2 rounded-lg bg-purple-600 text-white hover:bg-purple-700">{t('goToFarm')}</button>
