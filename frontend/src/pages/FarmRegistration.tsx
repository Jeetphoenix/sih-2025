...
const FarmRegistration: React.FC = () => {
  ...
  const [form, setForm] = useState({
    ...
    workers: [] as Array<{ name: string; age: string; role: string; contact: string }>
  })
  ...
  const handleWorkersCountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const getPoultryLabel = (value: string) => {
    switch (value) {
      case 'broiler':
        return t('poultryBroiler')
      case 'layer':
        return t('poultryLayer')
      case 'breeder':
        return t('poultryBreeder')
      case 'hatchery':
        return t('poultryHatchery')
      case 'backyard':
        return t('poultryBackyard')
      default:
        return value
    }
  }
    ...
    <div className="min-h-screen bg-gray-50">
      <header className="bg-green-600 text-white">
        <div className="max-w-5xl mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <h1 className="text-lg font-semibold">{t('farmRegistration')}</h1>
          </div>
      ...
      <main className="max-w-2xl md:max-w-5xl mx-auto px-4 py-6">
        ...
        >
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">{t('stateLabel')} <span className="text-red-500">*</span></label>
            <input
              type="text"
              name="state"
              value={form.state}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">{t('districtLabel')} <span className="text-red-500">*</span></label>
            <input
              type="text"
              name="district"
              value={form.district}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">{t('pinLabel')} <span className="text-red-500">*</span></label>
            <input
              type="text"
              name="pin"
              value={form.pin}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">{t('gpsCoordinates')}</label>
            <input
              type="text"
              name="gps"
              value={form.gps}
            ...
            <div className="flex items-center gap-3 mt-2">
              <button type="button" onClick={handleUseMyLocation} className="px-4 py-2 rounded-md bg-gray-100 hover:bg-gray-200 text-sm">
                {gpsLoading ? 'Getting location…' : t('useMyLocation')}
              </button>
              <p className="text-xs text-gray-500">{t('latLongFormat')}</p>
            </div>
            {errors.gps && <p className="mt-1 text-xs text-red-600">{errors.gps}</p>}
          </div>

          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">{t('typeOfPoultry')} <span className="text-red-500">*</span></label>
            <select
              name="poultryType"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">{t('currentFlockSize')}</label>
            <input
              type="number"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">{t('farmCapacity')}</label>
            <input
              type="number"
              name="farmCapacity"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">{t('vaccinatedQuestion')}</label>
            <select
              name="vaccinated"
          ...
          {form.vaccinated === 'yes' && (
            <>
              <div>
                <label className="block text-sm font-medium text-gray-800 mb-1">{t('lastVaccinatedDate')} <span className="text-red-500">*</span></label>
                <input
                  type="date"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-800 mb-1">{t('upcomingVaccinationDate')}</label>
                <input
                  type="date"
          ...
          <div>
            <label className="block text-sm font-medium text-gray-800 mb-1">{t('totalWorkers')}</label>
            <input
              type="number"
              name="workersCount"
          ...
          {Array.from({ length: parseInt(form.workersCount || '0', 10) || 0 }).map((_, i) => (
            <div key={i} className="md:col-span-2 border rounded-xl p-4 bg-gray-50">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-medium text-gray-800">{t('workerLabel')} {i + 1}</h4>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-800 mb-1">{t('nameLabel')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-800 mb-1">{t('ageLabel')}</label>
                  <input
                    type="number"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-800 mb-1">{t('roleLabel')}</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-800 mb-1">{t('contactInfo')}</label>
                  <input
                    type="tel"
          ...
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-800 mb-2">{t('uploadPhoto')}</label>
            <div className="flex items-center gap-3">
